{ config, pkgs, lib, ... }: with lib; let
  ...
  cfg = config.roos.user-mounts-generator;
in {
  options.roos.user-mounts-generator = with types; {
    enable = mkEnableOption ''
      Enable user-mounts-generator module.

      Users may want to mount some subvolumes under their home directory before
      the session is started. However, it is not always possible to know what
      subdirectories are desired at evaluation time.

      This module adds support for evaluating a "layout tree" containing btrfs
    ...
    configLines = mkOption {
      ...
      description = "user-mounts.yaml. By default, that generated by nixos.";
    };
  };
